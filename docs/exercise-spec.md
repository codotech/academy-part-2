...
1. Open `http://localhost:3000/api-docs` — verify Swagger UI loads and you can execute a search from the browser.
...
4. `docker compose up` — verify the backend responds on `localhost:3000`.
...
- `docs/walkthrough-laptop-to-production.md` — the "why" behind every step
- `docs/api-contracts.md` — deep dive on contracts, OAuth, and the OpenAPI ecosystem
- NotebookLM videos (links in Slack) — 7 deep-dives on git, Docker, CI, OpenAPI, env vars, Render, Claude+CI
