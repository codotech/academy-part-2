...
**Class:** Mon Apr 27 · **Cohort:** Pilot (4 students) · **Folder:** `syllabus/exercises/03-tooling-and-delivery/`
...
Those are not the same thing. Every junior engineer eventually hits this moment. You built something. It runs on your laptop. You're proud of it. Then someone asks: "Can we put this in production?" And you realize you don't know how to answer. Not because the code is bad — because the code is all you have.
...
**System tests** close the gap between "I manually tested it" and "I can prove it behaves correctly." In this course, we test from the outside only — no mocking internals, no unit tests for functions. A Testcontainers test that spins up the real container and hits the real endpoint is the only evidence that matters. This is the testing philosophy you will carry through the full Codo Academy course.

**CI** closes the gap between "the tests pass on my laptop" and "the tests pass on every commit, automatically, forever." A test suite that only runs when you remember to run it is not a guardrail. A GitHub Actions workflow that runs on every push — that is a guardrail.
...
- **You finished HW2.** Checkout `student/<your-name>`. Port your Spotify integration from your HW2 work onto this branch — cherry-pick, rebase, or copy your files. This is a real git exercise: moving code between branches deliberately.
...
## A Note on Video Order

The walkthrough above follows the spine: describe → run → prove → machines prove → world sees → AI helps.

The NotebookLM videos in `notebooklm/` are numbered by dependency order, not spine order. Video 1 (git) and video 2 (Docker) come first because they are prerequisites that every later video assumes. Video 4 (OpenAPI) comes after video 3 (CI) because the CI pipeline references OpenAPI-validated contracts. The ordering is intentional — watch them in numbered sequence.
...
- **`walkthrough-laptop-to-production.md`** — the guided walkthrough for class, the backbone reference
- **`api-contracts.md`** — deep dive on API contract design, Zod, OAuth, and the OpenAPI ecosystem
- **`notebooklm/`** — 7 video source outlines covering git, Docker, CI, OpenAPI, env vars, Render, and Claude+CI

Start with `exercise-spec.md`. Come to class on Apr 27 having read it.